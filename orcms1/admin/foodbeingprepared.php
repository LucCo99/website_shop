...
<head>
    ...
    <title>Online Railway Catering Management System||Food being prepared</title>
...
<body>
    ...
    $next_page = $page_no + 1;
    $adjacents = "2"; 
    $result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM tblorderaddresses where OrderFinalStatus='Food being Prepared'");
    $total_records = mysqli_fetch_array($result_count);
    $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; // total page minus 1
$ret=mysqli_query($con,"select * from tblorderaddresses where OrderFinalStatus='Food being Prepared' LIMIT $offset, $total_records_per_page");
$cnt=1;
...
<?php } else { ?>                  <td><?php  echo htmlentities($row['OrderFinalStatus']);?>
                  ...
                  <?php } ?>
                                    <td><a class="btn btn-success" href="viewfoodorder.php?orderid=<?php echo $row['Ordernumber'];?>">View Details</a>
                </tr>
                <?php 
$cnt=$cnt+1;
}?>
